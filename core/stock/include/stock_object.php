...
class stock_object extends object {
	...
	function remove_from_waiter ($order_id, $new_quantity) {
		...
		// moviments for all the contained ingredients
	...
	<table width="100%">
		...
		<tr>
			...
			<td>';
	if ($editing && $arr['ref_id']) {
		if($arr['ref_type']==TYPE_DISH) $class='dish';
		elseif($arr['ref_type']==TYPE_INGREDIENT) $class='ingredient';
		$obj = new $class;

		$output .= '
			<a href="'.$obj->file.'?class='.$class.'&command=edit&data[id]='.$arr['ref_id'].'">'.$this->name($_SESSION['language']).'</a>';
	} else {
		$output .= '
