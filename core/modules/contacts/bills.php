...
class bill extends module {
	function bill($id=0) {
		...
		$this -> addMenuLink(ucphr('BILLS'),'/admin/admin.php?class=bill&command=none',ucphr('BILLS').': '.ucphr('BILLS'));
		...
		$this->id=$id;
		$this -> title = ucphr('BILLS');
		$this -> label = ucphr('PRINTS_BILL');
...
*/
	...
	function post_insert ($input_data) {
		...
		$msg->setProperty('name',ucphr('BILL').' '.$input_data['internal_id']);
		$msg->setType(MSG_TYPE_ACCOUNTING);
		$modManager->distrMsg($msg);
