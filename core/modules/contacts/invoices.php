...
class invoice extends module {
	function invoice($id=0) {
		...
		$this -> addMenuLink(ucphr('INVOICES'),'/admin/admin.php?class=invoice&command=none',ucphr('INVOICES').': '.ucphr('INVOICES'));
		...
		$this->id=$id;
		$this -> title = ucphr('INVOICES');
		$this -> label = ucphr('INVOICE');
...
*/
	...
	function post_insert ($input_data) {
		...
		$msg->setProperty('name',ucphr('INVOICE').' '.$input_data['internal_id']);
		$msg->setType(MSG_TYPE_ACCOUNTING);
		$modManager->distrMsg($msg);
	...
	function post_delete ($input_data) {
		...
		$msg->setProperty('class',get_class($this));
		$msg->setProperty('id',$input_data['id']);
		$msg->setProperty('from',$input_data['from']);
		$msg->setProperty('to',$input_data['to']);
		$msg->setProperty('timestamp',$input_data['timestamp']);
		$msg->setProperty('transaction',$input_data['transaction']);
		$msg->setProperty('amount',$input_data['amount']);
		$msg->setProperty('name',ucphr('INVOICE').' '.$input_data['internal_id']);
