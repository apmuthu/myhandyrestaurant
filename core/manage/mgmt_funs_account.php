...
//echo $query;
	...
	$account_number=get_db_data(__FILE__,__LINE__,$_SESSION['mgmt_db'],'account_accounts','number',$account_id);
	$account_name=ucfirst(get_db_data(__FILE__,__LINE__,$_SESSION['mgmt_db'],'account_accounts','name',$account_id));
	$account_bank=ucfirst(get_db_data(__FILE__,__LINE__,$_SESSION['mgmt_db'],'account_accounts','bank',$account_id));
...
<?php
	...
	while($arr=mysql_fetch_array($res)){
		...
		if($arr['mgmt_id']){
			...
			$res_local=mysql_db_query($_SESSION['mgmt_db'],$query);
...
<table border="0">
	...
	<tr>
		<td>
		<FIELDSET>
		<LEGEND><?php echo phr('ACCOUNT_MAIN_LEGEND'); ?></LEGEND>
		...
		<table border="0">
			...
			<tr>
				<td>
				<input type="radio" onClick="document.account_main_form.submit()" name="command" value="list" checked>
				<a href="#" onclick="JavaScript:type_insert_check('account_main_form','command',0);document.account_main_form.submit();return(false);"><?php echo phr('ACCOUNT_LIST'); ?></a><br>
				</td>
			</tr>
			<tr>
				<td>
				<input type="radio" onClick="document.account_main_form.submit()" name="command" value="new">
				<a href="#" onclick="JavaScript:type_insert_check('account_main_form','command',1);document.account_main_form.submit();return(false);"><?php echo phr('ACCOUNT_INSERT'); ?></a><br>
				</td>
			</tr>
			<tr>
				<td>
				<input type="radio" onClick="document.account_main_form.submit()" name="command" value="movement_new">
				<a href="#" onclick="JavaScript:type_insert_check('account_main_form','command',2);document.account_main_form.submit();return(false);"><?php echo phr('ACCOUNT_MOVEMENT_INSERT'); ?></a><br>
				</td>
			</tr>
			</tbody>
		</table>
...
<td>
<fieldset>
<legend><?php echo phr('ACCOUNT_MOVEMENT_LEGEND'); ?></legend>

<form action="account.php" name="account_form" method="get">
<?php
	if($editing){
?>
<input type="hidden" name="command" value="movement_update">
<input type="hidden" name="id" value="<?php echo $id; ?>">
<?php
	} else {
?>
<input type="hidden" name="command" value="movement_insert">
<?php
	}
?>
<table>
	<tr>
		<td>
		<?php echo phr('ACCOUNT_LABEL_DATE'); ?>:
		</td>
		<td>
		<input type="text" size="2" maxlength="2" name="data[date][day]" value="<?php echo $day; ?>">/
		<input type="text" size="2" maxlength="2" name="data[date][month]" value="<?php echo $month; ?>">/
		<input type="text" size="4" maxlength="4" name="data[date][year]" value="<?php echo $year; ?>">
		</td>
	</tr>
	<tr>
		<td>
		<?php echo phr('ACCOUNT_LABEL_BANK_ACCOUNT'); ?>:
		</td>
		<td>
		<select name="data[account_id]">
<?php
		$table=$GLOBALS['table_prefix'].'account_accounts';
		$query="SELECT * FROM $table";
		$res_type=mysql_db_query($_SESSION['mgmt_db'],$query);
		while($arr_type=mysql_fetch_array($res_type)){
?>
			<option value="<?php echo $arr_type['id']?>" <?php if($arr_type['id']==$arr['account_id']) echo "selected"; ?>><?php echo $arr_type['bank']; ?>/<?php echo $arr_type['number']; ?> - <?php echo $arr_type['name']; ?></option>
<?php
		}
?>
		</select>
	...
	<tr>
		<td>
		<?php echo phr('ACCOUNT_LABEL_CAUSAL'); ?>:
		</td>
		<td>
...
<?php
		...
		$res_type=mysql_db_query($_SESSION['common_db'],$query);
		while($arr_type=mysql_fetch_array($res_type)){
...
?>
			<option value="<?php echo $arr_type['id']; ?>" <?php if($arr_type['id']==$arr['type']) echo "selected"; ?>><?php echo $type_name; ?></option>
<?php
		}
?>
		</select>
	...
	<tr>
		<td>
		<?php echo phr('ACCOUNT_LABEL_DESCRIPTION'); ?>:
		</td>
		<td>
		<input type="text" name="data[description]" value="<?php echo $arr['description']; ?>">
		</td>
	</tr>
	<tr>
		<td>
		<?php echo phr('ACCOUNT_LABEL_AMOUNT'); ?> (- <?php echo phr('ACCOUNT_LABEL_IF_NEGATIVE'); ?>):
		</td>
		<td>
		<input type="text" name="data[amount]" value="<?php echo $arr['amount']; ?>">
		</td>
	</tr>
	<tr>
...
if(!$editing){
?>
			<input type="submit" value="<?php echo phr('ACCOUNT_MOVEMENT_INSERT'); ?>">
</form>
			</td>
<?php
} else {
?>
			<td>
			<input type="submit" value="<?php echo phr('ACCOUNT_MOVEMENT_EDIT'); ?>">
</form>
			</td>
			<td>
			<form action="account.php" name="account_form" method="get">
			<input type="hidden" name="command" value="movement_delete">
			<input type="hidden" name="delete[<?php echo $id; ?>]" value="1">
			<input type="submit" value="<?php echo phr('ACCOUNT_MOVEMENT_DELETE'); ?>">
			</form>
			</td>
<?php
}
...
*/
?>
<?php echo phr('ACCOUNT_MOVEMENT'); ?> <b><?php echo $description; ?></b> <?php echo phr('ACCOUNT_MOVEMENT_DELETE_SUCCESS'); ?>.<br>
<?php
...
*/
?>
<?php echo phr('ACCOUNT_MOVEMENT_NOTHING_DONE'); ?>.<br>
<?php
		return $input_id;
	}
}
...
function account_movement_update($input_id,$input_data) {
	...
	$res_local=mysql_db_query($_SESSION['mgmt_db'],$query_local);
...
*/
?>
<?php echo phr('ACCOUNT_THE'); ?> <b><?php echo $input_data['name']; ?></b> <?php echo phr('ACCOUNT_MOVEMENT_UPDATE_SUCCESS'); ?>.<br>
<?php
	} elseif($errno=mysql_errno()) {
...
*/
?>
<?php echo phr('ACCOUNT_THE'); ?> <b><?php echo $input_data['name']; ?></b> <?php echo phr('ACCOUNT_MOVEMENT_ADD_SUCCESS'); ?>. <br>
<?php
	} elseif($errno=mysql_errno()) {
...
<table class="mgmt_color_tablebg">
	<thead>
		...
		<th><a href="account.php?command=list&orderby=currency"><?php echo phr('ACCOUNT_TABLE_CURRENCY'); ?></a></th>
...
?>
		<a href="supply.php?command=showknownsupplier&id=<?php echo $arr_bank['id']; ?>">
		<?php echo ucfirst($arr['bank']); ?>
		</a>
<?php
} else {
?>
...
?>
		...
		<td><a href="account.php?command=movement_list&id=<?php echo $arr['id']; ?>"><?php echo ucfirst($arr['name']); ?></a></td>
		<td><a href="account.php?command=movement_list&id=<?php echo $arr['id']; ?>"><?php echo $arr['number']; ?></a></td>
		<td><?php echo $arr['abi']; ?></td>
		<td><?php echo $arr['cab']; ?></td>
		<td><?php echo $arr['cin']; ?></td>
		<td><?php echo $arr['bic1']; ?></td>
		<td><?php echo $arr['bic2']; ?></td>
		<td><a href="account.php?command=movement_list&id=<?php echo $arr['id']; ?>"><?php echo $arr['iban']; ?></a></td>
