...
<?php
	...
	while($arr=mysql_fetch_array($res)) {
		$checked="";
		if(mysql_list_tables($arr['db']) && $_SESSION['mgmt_db']==$arr['db']) {
			$checked=" checked";
		}
		if(mysql_list_tables($arr['db'])) {
			if($account==$arr['db'])
...
function vat_report(){
	...
	echo "
	".ucfirst(phr('TAX_TO_PAY')).":<br><b>".country_conf_currency(true)." ".$vat_total."</b>
	 (".phr('EXCLUDING_NOT_PAID_INVOICES').")<br>
	".phr('OR')."<br>
	<b>".country_conf_currency(true)." ".$vat_total_with_debit."</b>  (".phr('INCLUDING_NOT_PAID_INVOICES').")<br><br>
	";
