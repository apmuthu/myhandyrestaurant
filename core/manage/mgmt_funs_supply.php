...
// next is only a ref line. to be deleted.
	...
	if(is_array($delete)){
		for (reset ($delete); list ($key, $value) = each ($delete); ) {
			...
			$description[$key]=get_db_data(__FILE__,__LINE__,$_SESSION['mgmt_db'],"account_mgmt_addressbook","name",$key);
			$counter++;
		}
	} else {
		$query.="`id`='$delete'";
		$description=get_db_data(__FILE__,__LINE__,$_SESSION['mgmt_db'],"account_mgmt_addressbook","name",$delete);
		$counter++;
	}
	// echo "<br>Query SQL:<br>".$query."<br>\n";
