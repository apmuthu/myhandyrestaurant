...
function movement_insert($data){
	...
	set_stock($data['name'],$data['quantity']);
...
function movement_update($data,$id){
	...
	$res2 = mysql_db_query ($_SESSION['mgmt_db'],$query);
	$num_affected = mysql_affected_rows();
...
function set_stock($dishname,$diffquantity){
	...
	$table=$GLOBALS['table_prefix'].'dishes';
	$query="UPDATE $table SET `stock`='".$newstock."' WHERE `id`='".$row['id']."'";
	$res=mysql_db_query($_SESSION['common_db'],$query);
	return 0;
}
...
?>
<h4>Scheda Movimento Magazzino</h4><br>
<table>
	<tr>
...
<?php
	if($row['invoice_id']) {
		$invoice_descr=get_db_data(__FILE__,__LINE__,$_SESSION['mgmt_db'],
...
<?php
	...
	while($row=mysql_fetch_array($res)){
		...
		if($row['invoice_id']) {
			...
			$res_local=mysql_db_query($_SESSION['mgmt_db'],"SELECT * FROM $table WHERE `id`='".$row['invoice_id']."'");
			if(mysql_num_rows($res_local)) {
				$row_local=mysql_fetch_array($res_local);
