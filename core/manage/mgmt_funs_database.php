...
function file_show($id){
	...
	*/
	...
	$res_supplier = mysql_db_query ($_SESSION['mgmt_db'],"SELECT * FROM $table WHERE `name`='".$row['who']."'");
...
function input_standard($id,$editing){
	...
	<td>Ora</td>
	<td>
		<input type=\"text\" size=\"2\" name=\"data[date][hour]\" value=\"$hour\"> :
		<input type=\"text\" size=\"2\" name=\"data[date][minute]\" value=\"$minute\"> :
		<input type=\"text\" size=\"2\" name=\"data[date][second]\" value=\"$second\">
	</td>
	</tr>
	";
	*/
	...
	$table=$GLOBALS['table_prefix'].'account_mgmt_addressbook';
	$res_supplier = mysql_db_query ($_SESSION['mgmt_db'],"SELECT * FROM $table ORDER BY `name`");
	if(!$editing){
		$selected="selected";
	} else {
		$selected="";
...
function display_form_invoice($id){
	...
	if($paid) {
		$table=$GLOBALS['table_prefix'].'account_mgmt_main';
		$res_payment=mysql_db_query($_SESSION['mgmt_db'],"SELECT * FROM $table WHERE `associated_invoice`='$id'");
	...
	if($editing==1 && $paid==1){
		$table=$GLOBALS['table_prefix'].'account_mgmt_main';
		$res=mysql_db_query($_SESSION['mgmt_db'],"SELECT * FROM $table WHERE `associated_invoice`='$id'");
		$row=mysql_fetch_array($res);
...
<select name="payment_data_account_id" <?php echo $disabled; ?>>
	...
	$res = mysql_db_query ($_SESSION['mgmt_db'],"SELECT * FROM $table ORDER BY `name`");
	$i=0;
	while($row=mysql_fetch_array ($res)) {
...
//DELETE FROM `account_mgmt_main` WHERE `id`='25' OR `id`='26';
	...
	for (reset ($delete); list ($key, $value) = each ($delete); ) {
		...
		$table=$GLOBALS['table_prefix'].'account_mgmt_main';
		$res=mysql_db_query($_SESSION['mgmt_db'],"SELECT * FROM $table WHERE `id`='$key'");
		$row=mysql_fetch_array($res);
...
//echo "Payment_data:<br>/n";var_dump($payment_data);echo "<br><br>/n";
	...
	if(get_db_data(__FILE__,__LINE__,$_SESSION['common_db'],'mgmt_types','log_to_bank',$input_data['type'])){
		...
		if($movement_id) {
			...
			$res2 = mysql_db_query ($_SESSION['mgmt_db'],$query2);
	...
	if ($num_affected==1) {
		...
		if($type=="fattura" && $input_data['paid']) {
			...
			$payment_data['description']=$invoice_data['description']." - Pagam.";
...
//print_r($data);
	...
	$old_type=get_db_data(__FILE__,__LINE__,$_SESSION['mgmt_db'],'account_mgmt_main','type',$input_id);
	$old_log_to_bank=get_db_data(__FILE__,__LINE__,$_SESSION['common_db'],'mgmt_types','log_to_bank',$old_type);
	$old_account_movement=get_db_data(__FILE__,__LINE__,$_SESSION['mgmt_db'],'account_mgmt_main','account_movement',$input_id);
...
function table_general($orderby="date",$commandto,$query_type=0,$query_value=0) {
	...
	*/
	...
	case 1:
		$table=$GLOBALS['table_prefix'].'account_mgmt_addressbook';
		$res=mysql_db_query($_SESSION['mgmt_db'],"SELECT * FROM $table WHERE `id`='$query_value'");
		$row=mysql_fetch_array($res);
...
<?php
	...
	while($row_type = mysql_fetch_array ($res_types)) {
		...
		$res = mysql_db_query ($_SESSION['mgmt_db'],$query_type);
		if(mysql_num_rows($res)) {
			$records_found=true;
		}
	}
